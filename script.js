const projects = [
  ...
  {slug:'moon-jar',title:'Moon Jar',tags:'Phone accessories · CMF',summary:'A phone-accessory concept study developed at Designskin, exploring how Korean visual identity could translate to an international market.',role:'Product design intern · Designskin',year:'2025',tools:'User and market research, material and CMF research, visual development, CAD and rendering',overviewTitle:'',overview:'Designskin wanted to retain a recognizable Korean identity while expanding internationally. I conducted user, market, and material research, then translated traditional ceramics, hardware, motifs, and embroidery techniques into phone-accessory concepts.',hero:'images/moon-jar/ceramic-forms-hd.png',heroAlt:'Four phone cases in white, pink, celadon green, and black with ceramic-inspired phone grips and embossed medallions',heroWide:true,heroDescription:'Moon jar-inspired phone grips explore rounded ceramic silhouettes and white, pink, celadon, and black finishes. Translucent, matte, and high-gloss surface studies test how ceramic depth and softness could translate to molded accessories, while embossed medallions add tactile detail to the cases.',hideImageLabels:true,galleryContain:true,detailImages:[{src:'images/moon-jar/metal-fittings-hd.png',alt:'White and black phone cases with metal phone grips based on Korean door and furniture hardware',caption:'Traditional hardware',description:'Metal phone-grip concepts reinterpret decorative fittings and pull rings found on traditional Korean doors and furniture.'},{src:'images/moon-jar/textile-patterns-hd.png',alt:'Four textile-like phone cases with crane and traditional Korean pattern variations',caption:'Motif and pattern study',description:'Pattern studies apply cranes and other traditional Korean motifs to textile-like case surfaces for a less literal cultural reference.'},{src:'images/moon-jar/illustrated-motifs-hd.png',alt:'Three phone cases with embroidered rabbit motifs and clothing-style name tags',caption:'Embroidery and personalization',description:'Embroidery studies highlight Designskin’s production technique through stitched character motifs and customizable clothing-style name tags.'}],processTitle:'Internship process',process:'Research findings were developed into four directions: ceramic-inspired CMF and embossing, metal grips based on traditional hardware, Korean motif studies, and embroidered personalization. Selected ideas were refined through CAD and rendering.',hideFinal:true},
...
function renderProject(slug){
  ...
  if(project.hero){
    ...
    if(project.heroDescription){
      const note=document.createElement('div');
      note.className='project-image-note';
      note.innerHTML=`<span>01</span><div><strong>Ceramic form and CMF</strong><p>${project.heroDescription}</p></div>`;
      hero.after(note);
    }
  }
  if(project.detailImages){
    ...
    grid.innerHTML=project.detailImages.map((image,index)=>`<figure class="project-gallery__item"><img src="${image.src}" alt="${image.alt}"><figcaption><span>${String(index+2).padStart(2,'0')}</span><div><strong>${image.caption}</strong>${image.description?`<p>${image.description}</p>`:''}</div></figcaption></figure>`).join('');
  }
  ...
  if(project.aiImages){
    const processStory=page.querySelectorAll('.story-block')[1];
    const fabricationGallery=page.querySelector('.process-grid');
    const aiSection=document.createElement('section');
    aiSection.className='ai-workflow';
    aiSection.innerHTML=`<div class="ai-workflow__intro"><p class="eyebrow">Workflow</p><div><h2>${project.aiTitle}</h2><p>${project.aiText}</p></div></div><div class="ai-workflow__grid">${project.aiImages.map(image=>`<figure><img src="${image.src}" alt="${image.alt}" loading="lazy"><figcaption>${image.caption}</figcaption></figure>`).join('')}</div>`;
    fabricationGallery.before(processStory);
    processStory.after(aiSection);
  }
  ...
  if(project.officialMedia){
    const nextNav=page.querySelector('.next-project');
    const media=document.createElement('section');
    media.className='official-media';
    media.innerHTML=`<div class="official-media__head"><p class="eyebrow">Official coverage</p><a href="${project.officialMedia.url}" target="_blank" rel="noopener noreferrer">View on Sulwhasoo <span aria-hidden="true">↗</span></a></div><div class="official-media__video"><iframe src="https://www.youtube-nocookie.com/embed/${project.officialMedia.youtubeId}" title="${project.officialMedia.title}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe></div>`;
    nextNav.before(media);
  }
